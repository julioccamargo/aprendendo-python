...
CRITERIOS = {
    'obrigatorios': {
        ' estoque': 15, ' recebimento': 10, ' armazenamento': 10,
        ' expedição': 10, ' controle de estoque': 10,
    },
    'desejaveis': {
        ' carga e descarga': 5, ' inventário': 10, ' logística': 4, ' organização': 3,
    }
}
...
PASTA_CURRICULOS = DIRETORIO_ATUAL / "curriculos_estoquista"
ARQUIVO_SAIDA_EXCEL = DIRETORIO_ATUAL / "Resultado_Triagem_ESTOQUISTA.xlsx"
ARQUIVO_HASHES = DIRETORIO_ATUAL / "hashes_processados_estoquista.txt"
...
def calcular_hash_arquivo(caminho_arquivo):
    ...
    try:
        with open(caminho_arquivo, 'rb') as f:
            while bloco := f.read(8192): sha256.update(bloco)
        return sha256.hexdigest()
    except IOError: return None

def carregar_hashes_processados(caminho_arquivo_hashes):
    if not caminho_arquivo_hashes.exists(): return set()
    with open(caminho_arquivo_hashes, 'r') as f: return set(line.strip() for line in f)

def salvar_hash_processado(caminho_arquivo_hashes, hash_valor):
    with open(caminho_arquivo_hashes, 'a') as f: f.write(hash_valor + '\n')
